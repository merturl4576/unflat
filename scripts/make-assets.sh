...
#   assets/hero.gif        — the forge-workshop demo wiping from before to after
...
W=720; H=1000
ffmpeg -y -loglevel error   -loop 1 -t 2.6 -i assets/05-before.png -loop 1 -t 4.0 -i assets/05-after.png   -filter_complex "[0]scale=$W:$H[b];[1]scale=$W:$H[a];[b][a]xfade=transition=wipedown:duration=1.4:offset=1.2[x];[x]pad=iw:ih+40:0:40:color=0x0f0b08,drawtext=fontfile='$FONT':text='before':x=16:y=12:fontsize=16:fontcolor=0x9a958d:enable='lt(t,1.9)',drawtext=fontfile='$FONT':text='after  ·  one CSS block, colors derived from the site':x=16:y=12:fontsize=16:fontcolor=0xffb454:enable='gte(t,1.9)',format=yuv420p" scratch/hero.mp4
ffmpeg -y -loglevel error -i scratch/hero.mp4   -vf "fps=12,split[s0][s1];[s0]palettegen=max_colors=256:stats_mode=diff[p];[s1][p]paletteuse=dither=sierra2_4a"   assets/hero.gif
rm -rf scratch
