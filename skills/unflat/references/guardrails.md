...
A site that already uses `body::before`: the block's light layer replaces it. Move the site's pseudo-element to `body::after` or put the light on `html::before` and say so in the report.
