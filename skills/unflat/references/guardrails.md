...
| Motion | the only motion is the hover lift on cards inside surfaces, under `prefers-reduced-motion: no-preference`, plus the one optional drift from taste.md §6 when it was chosen | grep the block for `animation` and `transition`: one `transition` (the lift), no `animation` unless the drift was chosen |
